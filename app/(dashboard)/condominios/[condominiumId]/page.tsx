...
export default async function CondominiumPage({ params }: { params: { condominiumId: string } }) {
  ...
  return (
    <>
      <PageHeader title={condominium.name} description={condominium.address || 'Gerencie as piscinas, medições, QR Codes e o modo compartilhar deste condomínio.'} actionLabel="Cadastrar piscina" actionHref={`/condominios/${condominium.id}/piscinas/nova`} />
          ...
          return (
            <article key={pool.id} className="card space-y-4">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <Link href={`/public/piscinas/${pool.slug}`} className="rounded-2xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700">Modo compartilhar</Link>
              </div>
            </article>
          );
        })}
